...
class spatial_MLR(object):
    ...
    def plot_regression_coef_map(self, path, alias, subplot_names, output_path):
        ...
        try:
            ...
            print('Hay problema con modelo '+alias)
        plt.clf()
...
def main(config):
    ...
    for alias, alias_list in meta.items():
        if (alias != 'MIROC-ES2L') & (alias!= 'NESM3'):
            ...
            try:
                ts_dict = {m["variable_group"]: xr.open_dataset(m["filename"])[m["short_name"]].sel(time=slice(start_str,end_str)).values for m in alias_list if (m["variable_group"] != "ua850") & (m["variable_group"] != "tas") & (m["variable_group"] != "pr") & (m["variable_group"] != "sst") & (m["variable_group"] != "psl") & (m["variable_group"] != "ta250") & (m["variable_group"] != 'ua50_spv')}
            ...
            new_column_order = ['ua50_spv', 'ta', 'tos_cp', 'tos_ep','gw']
            ...
            #MLR.perform_regression(config["work_dir"]+'/u850_RawDrivers_withZMremove/'+alias)
            #Plot coefficients
            os.chdir(config["plot_dir"])
            os.getcwd()
            os.makedirs("u850_RawDrivers_withZMremove",exist_ok=True)
            subplot_names = ['Climatology','Stratospheric \n Polar Vortex', 'Tropical \n Warming', 'Central Pacific \n aSST', 'Easter Pacific \n aSST','Global TAS']
            if alias != 'CNRM-CM6-1-HR':
                MLR.plot_regression_coef_map(config["work_dir"]+'/u850_RawDrivers_withZMremove',alias,subplot_names,config["plot_dir"]+'/u850_RawDrivers_withZMremove')
            else:
                print('arreglar esto')
    ...
    for alias, alias_list in meta.items():
        if (alias != 'MIROC-ES2L') & (alias!= 'NESM3'):
            ...
            os.makedirs(alias,exist_ok=True)
            MLR.perform_regression(config["work_dir"]+'/u850_RawDrivers_withoutZMremove/'+alias)
            #Plot coefficients
            os.chdir(config["plot_dir"])
            os.getcwd()
            os.makedirs("u850_RawDrivers_withoutZMremove",exist_ok=True)
            subplot_names = ['Climatology','Stratospheric \n Polar Vortex', 'Tropical \n Warming', 'Central Pacific \n aSST', 'Easter Pacific \n aSST','Global TAS']
            MLR.plot_regression_coef_map(config["work_dir"]+'/u850_RawDrivers_withoutZMremove',alias,subplot_names,config["plot_dir"]+'/u850_RawDrivers_withoutZMremove')
