...
class spatial_MLR(object):
    ...
    def plot_regression_coef_map(self, path, alias, output_path):
        ...
        for k, ax in enumerate(axs.flat):
            ...
            # Plot title
            ax.set_title(self.regressor_names[k], fontsize=18)
...
def main(config):
    ...
    for alias, alias_list in meta.items():
        if (alias != 'MIROC-ES2L') & (alias!= 'NESM3') & (alias!= 'CNRM-CM6-1-HR'):
            ...
            if start_year < 802:
                start_str = '0'+str(start_year)
                end_str = '0'+str(start_year+198)
            elif start_year < 1000 and start_year > 802:
                start_str = '0'+str(start_year)
                end_str = str(start_year+198)
            else:
                start_str = str(start_year)
                end_str = str(start_year+198)
            
            try:
                ts_dict = {m["variable_group"]: xr.open_dataset(m["filename"])[m["short_name"]].sel(time=slice(start_str,end_str)).values for m in alias_list if (m["variable_group"] != "ua850") & (m["variable_group"] != "tas") & (m["variable_group"] != "pr") & (m["variable_group"] != "sst") & (m["variable_group"] != "psl") & (m["variable_group"] != "ta250") & (m["variable_group"] != "gw")}
                ts_dict_tos_cp = {m["variable_group"]: xr.open_dataset(m["filename"])[m["short_name"]].sel(time=slice(start_str,end_str)).sel(lat=slice(-5,5)).sel(lon=slice(140,280)).mean(dim='lat').mean(dim='lon').values for m in alias_list if (m["variable_group"] == "sst")}
                ts_dict_tos_ep = {m["variable_group"]: xr.open_dataset(m["filename"])[m["short_name"]].sel(time=slice(start_str,end_str)).sel(lat=slice(0,10)).sel(lon=slice(270,285)).mean(dim='lat').mean(dim='lon').values for m in alias_list if (m["variable_group"] == "sst")}
                ...
                ts_dict['ua50_spv'] = ts_dict['ua50_spv'][:-1]
            ...
            min_length = min(len(value) for value in ts_dict.values())
            ...
            print(len(target_wind.time))
            MLR = spatial_MLR()
            MLR.regression_data(target_wind,pd.DataFrame(ts_dict).apply(stand,axis=0),pd.DataFrame(ts_dict).keys().insert(0,'clim'))
            ...
            maps, maps_pval, R2 = MLR.open_regression_coef(config["work_dir"]+'/'+"u850_RawDrivers_withZMremove_withoutGW"+'/'+alias)
            maps_mem.append(maps)
            #Plot coefficients
            os.chdir(config["plot_dir"])
            os.getcwd()
            os.makedirs("u850_RawDrivers_withZMremove_withoutGW",exist_ok=True)
        
    #ts_dict_keys = pd.DataFrame(ts_dict).keys().insert(0,'clim')
    #ensemble_maps = {}
    #for n,rd in enumerate(list(ts_dict_keys)):
    #    ensemble_maps[rd] = xr.concat([m[n] for m in maps_mem],dim='model')
        
    #ensemble_maps_mean = [ensemble_maps[rd].mean(dim='model') for rd in ensemble_maps.keys()]
    #ensemble_maps_pval = [xr.apply_ufunc(test_mean_significance,ensemble_maps[rd],input_core_dims=[["model"]],output_core_dims=[[]],vectorize=True,dask="parallelized") for rd in ensemble_maps.keys()]
    #print(ensemble_maps_mean)
    #plot_regression_coef_map_MEM(ensemble_maps_mean, ensemble_maps_pval , list(ensemble_maps.keys()), config["plot_dir"]+'/u850_RawDrivers_withZMremove_withoutGW/u850_RawDrivers_withZMremove_withoutGW_MEM_regression_coef.png')
    ...
    for alias, alias_list in meta.items():
        if (alias != 'MIROC-ES2L') & (alias!= 'NESM3') & (alias!= 'CNRM-CM6-1-HR'):
            ...
            try:
                ts_dict = {m["variable_group"]: xr.open_dataset(m["filename"])[m["short_name"]].sel(time=slice(start_str,end_str)).values for m in alias_list if (m["variable_group"] != "ua850") & (m["variable_group"] != "tas") & (m["variable_group"] != "pr") & (m["variable_group"] != "sst") & (m["variable_group"] != "psl") & (m["variable_group"] != "ta250")}
                ts_dict_tos_cp = {m["variable_group"]: xr.open_dataset(m["filename"])[m["short_name"]].sel(time=slice(start_str,end_str)).sel(lat=slice(-5,5)).sel(lon=slice(140,280)).mean(dim='lat').mean(dim='lon').values for m in alias_list if (m["variable_group"] == "sst")}
                ts_dict_tos_ep = {m["variable_group"]: xr.open_dataset(m["filename"])[m["short_name"]].sel(time=slice(start_str,end_str)).sel(lat=slice(0,10)).sel(lon=slice(270,285)).mean(dim='lat').mean(dim='lon').values for m in alias_list if (m["variable_group"] == "sst")}
                ts_dict_tos_zm = {m["variable_group"]: xr.open_dataset(m["filename"])[m["short_name"]].sel(time=slice(start_str,end_str)).sel(lat=slice(-10,10)).sel(lon=slice(0,360)).mean(dim='lat').mean(dim='lon').values for m in alias_list if (m["variable_group"] == "sst")}
                ts_dict['ua50_spv'] = ts_dict['ua50_spv'][:-1]
            ...
            os.chdir(config["work_dir"])
            os.getcwd()
            os.makedirs("u850_RawDrivers_withZMremove",exist_ok=True)
            os.chdir(config["work_dir"]+'/'+"u850_RawDrivers_withZMremove")
            os.makedirs(alias,exist_ok=True)
            maps, maps_pval, R2 = MLR.open_regression_coef(config["work_dir"]+'/'+"u850_RawDrivers_withZMremove"+'/'+alias)
            maps_mem.append(maps)
            #Plot coefficients
            os.chdir(config["plot_dir"])
            os.getcwd()
    ...
    ts_dict_keys = pd.DataFrame(ts_dict).keys().insert(0,'clim')
    print(len(ts_dict_keys))
    print(len(maps_mem[0]))
    ensemble_maps = {}
    for n,rd in enumerate(list(ts_dict_keys)):
        ensemble_maps[rd] = xr.concat([m[n] for m in maps_mem],dim='model')
    ...
    ensemble_maps_pval = [xr.apply_ufunc(test_mean_significance,ensemble_maps[rd],input_core_dims=[["model"]],output_core_dims=[[]],vectorize=True,dask="parallelized") for rd in ensemble_maps.keys()]
    print(ensemble_maps_mean)
    plot_regression_coef_map_MEM(ensemble_maps_mean, ensemble_maps_pval , list(ensemble_maps.keys()), config["plot_dir"]+'/u850_RawDrivers_withZMremove/u850_RawDrivers_withZMremove_MEM_regression_coef.png')
...
######## without ZM remove ----------------------------------
    for alias, alias_list in meta.items():
        if (alias != 'MIROC-ES2L') & (alias!= 'NESM3') & (alias!= 'CNRM-CM6-1-HR'):
            ...
            os.makedirs(alias,exist_ok=True)
            maps, maps_pval, R2 = MLR.open_regression_coef(config["work_dir"]+'/'+"u850_RawDrivers_withoutZMremove_withoutGW"+'/'+alias)
            maps_mem.append(maps)
            #Plot coefficients
            os.chdir(config["plot_dir"])
            os.getcwd()
    ...
    ensemble_maps_pval = [xr.apply_ufunc(test_mean_significance,ensemble_maps[rd],input_core_dims=[["model"]],output_core_dims=[[]],vectorize=True,dask="parallelized") for rd in ensemble_maps.keys()]
    print(ensemble_maps_mean)
    plot_regression_coef_map_MEM(ensemble_maps_mean, ensemble_maps_pval , list(ensemble_maps.keys()), config["plot_dir"]+'/u850_RawDrivers_withoutZMremove_withoutGW/u850_RawDrivers_withoutZMremove_withoutGW_MEM_regression_coef.png')
    ...
    for alias, alias_list in meta.items():
        if (alias != 'MIROC-ES2L') & (alias!= 'NESM3') & (alias!= 'CNRM-CM6-1-HR'):
            ...
            os.chdir(config["work_dir"])
            os.getcwd()
            os.makedirs("u850_RawDrivers_withoutZMremove",exist_ok=True)
            os.chdir(config["work_dir"]+'/'+"u850_RawDrivers_withoutZMremove")
            os.makedirs(alias,exist_ok=True)
            maps, maps_pval, R2 = MLR.open_regression_coef(config["work_dir"]+'/'+"u850_RawDrivers_withoutZMremove"+'/'+alias)
            maps_mem.append(maps)
            #Plot coefficients
            os.chdir(config["plot_dir"])
            os.getcwd()
    ...
    ensemble_maps_pval = [xr.apply_ufunc(test_mean_significance,ensemble_maps[rd],input_core_dims=[["model"]],output_core_dims=[[]],vectorize=True,dask="parallelized") for rd in ensemble_maps.keys()]
    print(ensemble_maps_mean)
    plot_regression_coef_map_MEM(ensemble_maps_mean, ensemble_maps_pval , list(ensemble_maps.keys()), config["plot_dir"]+'/u850_RawDrivers_withoutZMremove/u850_RawDrivers_withoutZMremove_MEM_regression_coef.png')
